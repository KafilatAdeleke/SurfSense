...
class ZendeskConnector:
    def __init__(self, subdomain: str, email: str, api_token: str):
        ...
        creds = {"email": email, "token": api_token, "subdomain": subdomain}
    ...
    async def get_tickets(self, limit: int = 1000) -> list[dict[str, Any]]:
        ...
        try:
            ...
            ticket_generator = self.client.search_export(
                type="ticket", status_less_than="closed"
            )

            for count, ticket in enumerate(ticket_generator):
                if count >= limit:
                    break
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not fetch comments for ticket {ticket.id}: {e}"
                    )
    ...
    async def get_help_center_articles(self, limit: int = 1000) -> list[dict[str, Any]]:
        ...
        articles = []
        try:
            for count, article in enumerate(self.client.help_center.articles()):
                if count >= limit:
                    break
